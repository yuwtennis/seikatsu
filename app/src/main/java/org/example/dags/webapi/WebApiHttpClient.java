...
public class WebApiHttpClient implements Caller<WebApiHttpRequest, WebApiHttpResponse> {
  ...
  public WebApiHttpResponse call(final WebApiHttpRequest webApiHttpRequest)
      throws UserCodeExecutionException {
    try {
      LOG.info("Url: {}", webApiHttpRequest.getUrl());
      ...
      return WebApiHttpResponse.builder()
          .setData(data)
          .setCategory(webApiHttpRequest.getCategory())
          .build();

    } catch (Exception e) { // IOException, RuntimeException, HttpResponseException
      e.printStackTrace();
